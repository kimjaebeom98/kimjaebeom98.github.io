...
일단 dynamic programming 다운 생각을 해야해서
이전에 했던 계산을 저장하여 이후에 사용하는 방법을 어떻게든 생각을 해야했다.

그리하여, loop한 턴 마다 그 턴의 max값을 정하여 저장해야한다.

그렇게 하기 위해서는 inner for문이 하나 더 필요했고, inner for문에 들어가기 전 dp[i]에는 i개의 카드팩을 가진 값을 저장하였고,
inner for문 안에서는 dp값을 계속 갱신하여 값을 비교하였음
